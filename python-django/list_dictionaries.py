...
#     transdetails = account["transactions"]
#     for details in transdetails:
#         if (details["method"] == "g-pay"):
#             print(details)
#
print([details for account in accounts for details in account["transactions"] if (details["method"])=="g-pay"])

#  print all transaction with amount > 300
print([details for account in accounts for details in account["transactions"] if (details["amount"])>=300])

#print highest balance account
max_bal=max(accounts,key=lambda a:a["balance"])
print(max_bal)

#sort accounts based on balance order by desc
accounts.sort(key=lambda account:account["balance"],reverse=True)

#sum of balance account
totalsum=sum([account["balance"]for account in accounts])
print(totalsum)